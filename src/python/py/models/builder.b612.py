...
class Model:
    ...
    def make_matmul_fp16_or_fp32(self, matmul, name, root_input, **kwargs):
        print("make_matmul_fp16_or_fp32: name = ", name)
        ...
        last_dim = 0
        m_weight = matmul.weight
        if hasattr(m_weight, "shape"):
            last_dim = matmul.weight.shape[0]
        elif hasattr(m_weight, "tensor"):
            last_dim = matmul.weight.tensor.shape[0]
        else:
            # error
            last_dim = 0
    ...
    def make_packed_matmul_fp16_or_fp32(self, q_matmul, k_matmul, v_matmul, name, root_input, **kwargs):
        ...
        N_q, H = q_matmul.weight.tensor.shape
        N_kv, _ = k_matmul.weight.tensor.shape

        # Create dummy PackedMatMul class
        class PackedMatMul:
            def __init__(self):
                # self.weight = torch.concatenate([q_matmul.weight.tensor.detach().cpu(), k_matmul.weight.tensor.detach().cpu(), v_matmul.weight.tensor.detach().cpu()], dim=0).reshape(N_q + N_kv + N_kv, H)
                # Convert memmap objects to PyTorch tensors
                q_tensor = torch.tensor(q_matmul.weight.tensor)
                k_tensor = torch.tensor(k_matmul.weight.tensor)
                v_tensor = torch.tensor(v_matmul.weight.tensor)

                # Detach and move to CPU
                q_tensor = q_tensor.detach().cpu()
                k_tensor = k_tensor.detach().cpu()
                v_tensor = v_tensor.detach().cpu()

                # Concatenate and reshape
                self.weight = torch.concatenate([q_tensor, k_tensor, v_tensor], dim=0).reshape(N_q + N_kv + N_kv, H)                
        matmul = PackedMatMul()
    ...
    def make_attention(self, layer_id, attention, root_input, **kwargs):
        ...
        if (attention.q_proj.bias is not None):
            q_bias_tensor = torch.tensor(attention.q_proj.bias.numpy())
            if (torch.count_nonzero(q_bias_tensor) > 0):
                q_bias_exists = True
        ...
        if (attention.k_proj.bias is not None):
            k_bias_tensor = torch.tensor(attention.k_proj.bias.numpy())
            if (torch.count_nonzero(k_bias_tensor) > 0):
                k_bias_exists = True
        ...
        if (attention.v_proj.bias is not None):
            v_bias_tensor = torch.tensor(attention.v_proj.bias.numpy())
            if (torch.count_nonzero(v_bias_tensor) > 0):
                v_bias_exists = True
    ...
    def make_mlp_fc(self, layer_id, mlp, root_input):
        ...
        fc1_bias_exists = False
        if (mlp.fc1.bias is not None):
            fc1_bias_tensor = torch.tensor(mlp.fc1.bias.numpy())
            if (torch.count_nonzero(fc1_bias_tensor) > 0):
                fc1_bias_exists = True
        # fc2_bias_exists = mlp.fc2.bias is not None and torch.count_nonzero(mlp.fc2.bias) > 0
        fc2_bias_exists = False
        if (mlp.fc2.bias is not None):
            fc2_bias_tensor = torch.tensor(mlp.fc2.bias.numpy())
            if (torch.count_nonzero(fc2_bias_tensor) > 0):
                fc2_bias_exists = True
    ...
    def make_layer(self, layer_id, layer):
        ...
        print("make_layer: first_layernorm")
        ...
        print("make_layer: self_attn - output_0")
        self.make_attention(layer_id, layer.self_attn, root_input=self.layernorm_attrs["output_0"])
        print("make_layer: post_attn_layernorm")
